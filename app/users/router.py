...
async def register_user(user_data: SUserRegister) -> dict:
    user = await UsersDAO.find_one_or_none(user_name=user_data.user_name)
    if user:
        raise HTTPException(
...
async def auth_user(response: Response, user_data: SUserAuth):
    check = await authenticate_user(user_name=user_data.user_name, password=user_data.password)
    if check is None:
